...
def _parse_gradle_modules(settings_path: Path, lightweight: bool = False) -> List[dict]:
    ...
    # Support both Groovy and Kotlin DSL forms, including multiple modules:
    # include ':app'
    # include(":app")
    # include(":feature:home", ":lib:core")
    pattern = re.compile(
        r"^\s*(?:includeFlat|include)\b\s*(?:\((?P<paren>[^\)]*)\)|(?P<plain>[^\n]*))",
        re.MULTILINE,
    )
    seen = set()
    for match in pattern.finditer(content):
        ...
        if "//" in line:
            continue
        args = match.group("paren") if match.group("paren") is not None else match.group("plain")
        for raw_match in re.finditer(r"['\"]([^'\"]+)['\"]", args):
            raw = raw_match.group(1).lstrip(":")
...
def _parse_build_config(module_path: Path) -> dict:
    ...
    if gradle_file:
        ...
        if "com.android.application" in content or "libs.plugins.android.application" in content:
            config["plugin_type"] = "application"
        ns = re.search(r'namespace\s*=\s*["\']([^"\']+)["\']', content)
        if ns:
            config["namespace"] = ns.group(1)
        app_id = re.search(r'applicationId\s*=\s*["\']([^"\']+)["\']', content)
        if app_id:
            config["application_id"] = app_id.group(1)
        compile_sdk = re.search(r'compileSdk\s*=\s*(\d+)', content)
        if not compile_sdk:
            compile_sdk = re.search(r'compileSdk\s*\{[^}]*version\s*=\s*release\((\d+)\)', content, re.DOTALL)
        if compile_sdk:
            config["compile_sdk"] = compile_sdk.group(1)
        min_sdk = re.search(r'minSdk\s*=\s*(\d+)', content)
        if min_sdk:
            config["min_sdk"] = min_sdk.group(1)
        target_sdk = re.search(r'targetSdk\s*=\s*(\d+)', content)
        if target_sdk:
            config["target_sdk"] = target_sdk.group(1)
        config["compose"] = bool(re.search(r'compose\s*=\s*true', content))
        for m in re.finditer(
            r"(implementation|api|compileOnly|runtimeOnly)\s*\(?\s*project\(\s*['\"][:']?([^'\"]+)['\"]", content
        ):
            config["dependencies"].append({"name": m.group(2).lstrip(":"), "type": m.group(1)})

        # 外部关键依赖
        for m in re.finditer(
            r"(implementation|api)\s*\(?\s*['\"]([^'\"]+:[^'\"]+:[^'\"]+)['\"]", content
        ):
            config["dependencies"].append({"name": m.group(2), "type": m.group(1), "external": True})
        for m in re.finditer(
            r"(implementation|api|testImplementation|androidTestImplementation|debugImplementation|compileOnly|runtimeOnly)\s*\(\s*(?:platform\(\s*)?libs\.([A-Za-z0-9_.]+)",
            content,
        ):
            config["dependencies"].append({"name": f"libs.{m.group(2)}", "type": m.group(1), "external": True})
